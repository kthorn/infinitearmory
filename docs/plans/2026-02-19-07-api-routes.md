...
**Prerequisites:** Components 2 (Database), 3 (WeaponSpec schemas), and 6 (Generation pipeline) must be complete. Component 10 (Auth/Rate Limiting) adds security hardening on top of these routes.
...
- Create: `src/app/api/weapons/__tests__/weapon-id.test.ts`
- Create: `src/app/api/weapons/__tests__/regenerate-image.test.ts`
- Create: `src/app/api/weapons/__tests__/reroll-stats.test.ts`
- Create: `src/app/api/health/__tests__/route.test.ts`

**Step 1: Create weapons route handler tests**
...
**Step 2: Create weapon-by-id route tests**

Create file `src/app/api/weapons/__tests__/weapon-id.test.ts`:
...
**Step 3: Create regenerate-image route tests**

Create file `src/app/api/weapons/__tests__/regenerate-image.test.ts`:

```typescript
import { describe, it, expect, vi, beforeEach } from 'vitest'
import { NextRequest } from 'next/server'
import { POST } from '../../[id]/regenerate-image/route'
...
**Step 4: Create reroll-stats route tests**

Create file `src/app/api/weapons/__tests__/reroll-stats.test.ts`:

```typescript
import { describe, it, expect, vi, beforeEach } from 'vitest'
import { NextRequest } from 'next/server'
import { POST } from '../../[id]/reroll-stats/route'
...
Create file `src/app/api/health/__tests__/route.test.ts`:
...
Run:
```bash
git add src/app/api/weapons/__tests__ && git add src/app/api/health/__tests__ && git commit -m "test: add API route handler tests"
```
